...
public class Indexer {
	...
	/**
	 * 建立新索引文件 jieba分词的search模式
	 * 
	 * @author WC
	 */
	public static void luceneCreateIndex_jiebasearchmode() throws Exception {
		// 指定新索引存放的位置
		Directory directory = FSDirectory.open(Paths.get(new File("D:\\Lucene_Path\\Lucene_jiebasearch").getPath()));
		System.out.println("pathname" + Paths.get(new File("D:\\Lucene_Path\\Lucene_jiebasearch").getPath()));
		// 创建一个分词器
		// StandardAnalyzer analyzer = new StandardAnalyzer();
		// CJKAnalyzer cjkAnalyzer = new CJKAnalyzer();
		// 智能中文分词器
		// SmartChineseAnalyzer smartChineseAnalyzer = new SmartChineseAnalyzer();
		JiebaAnalyzer jiebaAnalyzer = new JiebaAnalyzer(JiebaSegmenter.SegMode.SEARCH);
		// 创建indexwriterConfig(参数分词器)
		IndexWriterConfig indexWriterConfig = new IndexWriterConfig(jiebaAnalyzer);
		// 创建indexwriter对象(文件对象,索引配置对象)
		IndexWriter indexWriter = new IndexWriter(directory, indexWriterConfig);
	...
	public static void multi_search(String field, String[] content, int dir) throws IOException {
		...
		if (dir == 1) {
			directory = FSDirectory.open(Paths.get(new File("D:\\Lucene_Path\\Lucene_index\\accurate_index").getPath()));
		}
		// 创建indexReader对象
		IndexReader indexReader = DirectoryReader.open(directory);
		// 创建indexSearcher对象
...
//				System.out.println(name);
	...
	public static void main(String[] args) throws Exception {
		// luceneCreateIndex_jiebasearchmode();
		String[] content = { "无线" };
		multi_search("abstract", content, 1);
	}
}